      ...
      $.ajax({
        ...
        success: function (response) {
          ...
          if (response.data.length !== 0) {
            $.each(response.data, function (i, v) {
              $('#data').append('<button class="codingasik-produk" id="btn-link" onclick="getDetail(\'' + v.links + '\')"><img class="d-block img-produk" src="' + v.img + '"><div class="w-100"><p id="prod-name" class="text-start name nama-item">' + v.names + '</p><div class="d-flex"><p class="m-0 text-start harga-item">' + v.prices + '</p><p class="m-0 diskon" id="d' + i + '"> <span class="text-decoration-line-through fw-bold ">' + v.price_old + '</span> <span> -' + v.discount + ' OFF</span>  </p></div></div><div class="arrow"><i class="fas fa-angle-right"></i></div> </button>')
